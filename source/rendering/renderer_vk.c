#include "pch.h"
    ...
    VkDeviceMemory colorMSAAMemory = VK_NULL_HANDLE;
    ...
    extern VkSemaphore imageAvailable[RENDER_MAX_FRAMES_IN_FLIGHT];
    extern VkSemaphore renderFinished[RENDER_MAX_FRAMES_IN_FLIGHT];
    extern VkFence inFlightFences[RENDER_MAX_FRAMES_IN_FLIGHT];
    extern VkCommandBuffer commandBuffers[RENDER_MAX_FRAMES_IN_FLIGHT];
    ...
    static void create_color_msaa_resources()
    {
    ...
    static void create_camera_ubo(void)
    {
        ...
        create_vulkan_buffer(size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, &cameraUBOBuffer, &cameraUBOMemory);
    }
    ...
    {
        if (g_ui_fb) {
            free(g_ui_fb);           // avoid memory leak if called multiple times
        }
        ...
        allocInfo.commandBufferCount = RENDER_MAX_FRAMES_IN_FLIGHT;  // allocate N frames
        ...
        for (int i = 0; i < RENDER_MAX_FRAMES_IN_FLIGHT; i++) {
