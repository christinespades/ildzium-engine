...
void upload_ui_to_vulkan(uint32_t* ui_pixels, uint32_t width, uint32_t height) {
    // 1. Create staging buffer
    VkDeviceSize bufferSize = swapchainExtent.width * swapchainExtent.height * sizeof(uint32_t);
...
void draw_frame()
{
    ...
    if (ui_ctx.cursor_captured) {
        ...
        // Bind UI pipeline and draw quad over scene
        ...
        vkCmdBindDescriptorSets(commandBuffers[currentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS,
                                uiPipelineLayout, 0, 1, &uiDescriptorSet, 0, NULL);
        vkCmdDraw(commandBuffers[currentFrame], 4, 1, 0, 0);
    }
